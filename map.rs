...
pub const MAX_LAT: f64 = 85.05112877980660;
...
impl MapState {
    ...
    fn request_tiles(&self, win_w: f32, win_h: f32) {
        ...
        let mut push = |keys: &mut Vec<TileKey>, level: u8, cx: f64, cy: f64, scale: f64| {
        ...
        for k in keys {
            match self.tiles.get(&k) {
                ...
                Some(t) if t.failed_at.map_or(false, |at| at.elapsed() < RETRY_AFTER) => {}
                _ => fresh.push(k),
            }
        }
